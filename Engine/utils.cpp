...
void utils::DrawRect( float left, float bottom, float width, float height, float lineWidth )
{
	...
	//fixed the outline corners not being filled in
	//and changed the outline to go on the inside because it's easier to calculate with
	if (width > 0 && height > 0 && lineWidth > 0)
	{
		FillRect(left, bottom, width, lineWidth);
		FillRect(left + width - lineWidth, bottom, lineWidth, height);
		FillRect(left, bottom + height - lineWidth, width, lineWidth);
		FillRect(left, bottom, lineWidth, height);
	}
}
